...
`,
    ...
    function createSuspenseBoundary(
        ...
        if (isSuspensible) {
            if (
                parentSuspense == null ? void 0 : parentSuspense.pendingBranch
            ) {
    ...
    function doWatch(
        ...
        const instance =
            getCurrentScope() ===
            ((_a = currentInstance) == null ? void 0 : _a.scope)
                ? currentInstance
                : null;
        let getter;
...
If you want to remount the same app, move your app creation logic into a factory function and create fresh app instances for each mount - e.g. \`const createMyApp = () => createApp(App)\``
    ...
    function getType(ctor) {
        const match =
            ctor && ctor.toString().match(/^\s*(function|class) (\w+)/);
        ...
        if (!context.inVPre) {
            if (tag === "slot") {
                ...
                if (
                    props.some(
                        (p) =>
                            p.type === 7 && isSpecialTemplateDirective(p.name)
                    )
                ) {
                    tagType = 3;
                }
        ...
        if (!context.inVPre && /^(v-[A-Za-z0-9-]|:|\.|@|#)/.test(name)) {
            ...
            if (match2[2]) {
                ...
                const startOffset = name.lastIndexOf(
                    match2[2],
                    name.length -
                    (((_a = match2[3]) == null ? void 0 : _a.length) || 0)
                );
...
`);
                    ...
                    if (isTemplate) {
                        node.children.some((c) => {
                            if (c.type === 1) {
                                ...
                                if (key) {
                                    context.onError(
                                        createCompilerError(33, key.loc)
                                    );
                                    return true;
                                }
                            }
    ...
    function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
        ...
        if (!onComponentSlot) {
            ...
            if (!hasTemplateSlots) {
                slotsProperties.push(
                    buildDefaultSlotProperty(void 0, children)
                );
            } else if (
            ...
            if (prop.type === 6) {
                ...
                if (!arg && (isVBind || isVOn)) {
                    ...
                    if (exp) {
                        ...
                        context.onError(
                            createCompilerError(isVBind ? 34 : 35, loc)
                        );
                    }
                    continue;
                }
                ...
                if (directiveTransform) {
                    ...
                    if (isVOn && arg && !isStaticExp(arg)) {
                        pushMergeArg(
                            createObjectExpression(props2, elementLoc)
                        );
                    } else {
                    ...
                    if (type.type === 7) {
                        ...
                        switch (type.value.content) {
                            ...
                            case "file":
                                isInvalidType = true;
                                context.onError(
                                    createDOMCompilerError(59, dir.loc)
                                );
                                break;
                            default:
